...
TABLES = ["dim_lead", "dim_seller", "dim_business_segment", "dim_date", "fact_deal"]
...
def load_table(conn, name: str) -> int:
    ...
    success, n_chunks, n_rows, _ = write_pandas(
        conn, df, table_name=name.upper(), database="MERCHANT_FUNNEL_WAREHOUSE", schema="ANALYTICS"
    )
    if not success:
...
def main() -> None:
    ...
    try:
        ...
        cur.execute(
            ...
            FROM MERCHANT_FUNNEL_WAREHOUSE.ANALYTICS.fact_deal f
            GROUP BY f.lead_type
